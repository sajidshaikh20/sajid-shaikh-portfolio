...
const initVideoPlayer = function() {
  const video = document.getElementById('nike-video');
  const playBtn = document.getElementById('nike-play-btn');
  const muteBtn = document.getElementById('nike-mute-btn');
  const muteIcon = document.getElementById('nike-mute-icon');
  const progressFill = document.getElementById('nike-progress');
  const progressBar = progressFill?.parentElement;
  const videoWrapper = video?.closest('.project-video-wrapper');

  if (!video || !playBtn || !muteBtn || !progressFill) return;
  ...
  video.addEventListener('volumechange', function() {
    if (video.muted || video.volume === 0) {
      ...
      muteIcon.setAttribute('name', 'volume-low');
    } else {
      muteIcon.setAttribute('name', 'volume-high');
    }
  });
